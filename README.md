...
当前版本已经不再是早期的单桌游原型，而是一个以 `catalog + game.json + 远端资源缓存` 为核心的数据驱动应用。现在目录中已接入 `21` 款桌游，并支持按资源源优先级从远端拉取图片、文档和桌游元数据。
...
- `assets/games/<slug>/game.json`
  - 控制单款桌游的标题、简介、图片、文档入口、知识库入口和本地展示文案
- 远端资源库
  - 提供图片、规则书、FAQ、Markdown 摘要和远端 manifest
- 本地缓存
...
- 桌游 manifest 启动时优先使用缓存或包内资源
- 文档资源不再内置进 APK，而是按需下载缓存
